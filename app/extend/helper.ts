...
export namespace mongoose {
    ...
    export function query<T extends LeanDocument<{}>, P extends Partial<{
        ...
        const select: Nullable<Array<string>> = Array.isArray(projection?.select)
            ? ensure(projection?.select).filter(Boolean)
            : typeof projection?.select === "string"
                ? projection.select.split(' ').filter(Boolean)
                : null;
        ...
        for (let i = 0; i < length; i++) {
            ...
            for (let k = 0; k < conditionKeys.length; k++) {
                ...
                const comparator = (typeof record[key] === "object" && record[key] !== null) && record[key]._id?.toString() || record[key];
                ...
                if (typeof filter === "object") {
                    ...
                    skipRecord = (filter !== comparator) || skipRecord;
                }
