...
/**
 ** @description - Remove some specified properties from source object.
 ** @template T
...
const omit = (keys, object) => {
    ...
    const destObj = Object.assign({}, object);
    for (let i = 0; i < numKeys; i++) {
        let curr = destObj;
        const subKeys = keys[i].split('.'); // Ex: [1,2,3]
        const lastKey = subKeys.splice(-1); // Ex: ${lastKey}==3; ${subKeys}==[1,2]
...
const __main__ = () => {
    const exports = (removeKeys) => omit(removeKeys, {
        generatorConfig: {
            model: {
    ...
    return exports(['generatorConfig.mongoose']);
};
